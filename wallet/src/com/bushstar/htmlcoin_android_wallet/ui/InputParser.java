...
import com.bushstar.htmlcoinj.core.DumpedPrivateKey;
...
import com.bushstar.htmlcoinj.protocols.payments.PaymentRequestException;
import com.bushstar.htmlcoinj.protocols.payments.PaymentRequestException.PkiVerificationException;
import com.bushstar.htmlcoinj.uri.HTMLcoinURI;
import com.bushstar.htmlcoinj.uri.HTMLcoinURIParseException;
...
public abstract class InputParser
{
	...
	public abstract static class StringInputParser extends InputParser
	{
		...
		public void parse()
		{
			...
			else if (input.startsWith("htmlcoin:"))
			{
				try
				{
					final HTMLcoinURI htmlcoinUri = new HTMLcoinURI(null, input);

					final Address address = htmlcoinUri.getAddress();
					if (address == null)
						throw new HTMLcoinURIParseException("missing address");

					if (address.getParameters().equals(Constants.NETWORK_PARAMETERS))
						handlePaymentIntent(PaymentIntent.fromHTMLcoinUri(htmlcoinUri));
					else
						error(R.string.input_parser_invalid_address, input);
				}
				catch (final HTMLcoinURIParseException x)
				{
					log.info("got invalid htmlcoin uri: '" + input + "'", x);

					error(R.string.input_parser_invalid_htmlcoin_uri, input);
				}
			}
			else if (PATTERN_HTMLcoin_ADDRESS.matcher(input).matches())
			{
				try
				{
	...
	private static final Pattern PATTERN_HTMLcoin_ADDRESS = Pattern.compile("[" + new String(Base58.ALPHABET) + "]{20,40}");
