...
package com.bushstar.htmlcoin_android_wallet.integration.android;
 ...
 */
public final class HTMLcoinIntegration
{
	...
	private static final String MIMETYPE_PAYMENTREQUEST = "application/htmlcoin-paymentrequest"; // BIP 71

	/**
	 * Request any amount of HTMLcoin (probably a donation) from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	{
		final Intent intent = makeHTMLcoinUriIntent(address, null);

		start(context, intent);
	}

	/**
	 * Request specific amount of HTMLcoin from user, without feedback from the app.
	 * 
	 * @param context
	 *            Android context
	 * @param address
	...
	/**
	 * Request any amount of HTMLcoin (probably a donation) from user, with feedback from the app. Result intent can be
	 ...
	 * 
	 * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own HTMLcoin
	...
	/**
	 * Request specific amount of HTMLcoins from user, with feedback from the app. Result intent can be received by
	...
	/**
	 * Put BIP70 payment message into result intent. Meant for usage by HTMLcoin wallet applications.
	 * 
	 * @param result
	 *            result intent
	 * @param payment
	...
	/**
	 * Get BIP70 payment message from result intent. Meant for usage by applications initiating a HTMLcoin payment.
	 * 
	 * You can use the transactions contained in the payment to validate the payment. For this, you need your own
	 * HTMLcoin infrastructure though. There is no guarantee that the payment will ever confirm.
	 * 
	 * @param result
	 *            result intent
	...
	/**
	 * Put transaction hash into result intent. Meant for usage by HTMLcoin wallet applications.
	 * 
	 * @param result
	 *            result intent
	 * @param txHash
	...
	/**
	 * Get transaction hash from result intent. Meant for usage by applications initiating a HTMLcoin payment.
	 * 
	 * You can use this hash to request the transaction from the HTMLcoin network, in order to validate. For this, you
	 * need your own HTMLcoin infrastructure though. There is no guarantee that the transaction has ever been broadcasted
	 * to the HTMLcoin network.
	 * 
	 * @param result
	 *            result intent
	...
	private static Intent makeHTMLcoinUriIntent(final String address, final Long amount)
	{
		final StringBuilder uri = new StringBuilder("htmlcoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No HTMLcoin application found.\nPlease install HTMLcoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=com.bushstar.htmlcoin_android_wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/Bushstar/htmlcoin-android-wallet"));
